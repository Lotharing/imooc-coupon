...
public class UserServiceImpl implements IUserService {
    ...
    public List<Coupon> findCouponsByStatus(Long userId, Integer status) throws CouponException {
        ...
        if (CollectionUtils.isNotEmpty(curCached)) {
            ...
            // 微服务调用 填充 dbCoupons的 templateSDK 字段
        ...
        if (CouponStatus.of(status) == CouponStatus.USABLE) {
            ...
            if (CollectionUtils.isNotEmpty(classify.getExpired())) {
                ...
                //已经过期的状态,过期的优惠卷Ids
                CouponKafkaMessage couponKafkaMessage = new CouponKafkaMessage(CouponStatus.EXPIRED.getCode(), classify.getExpired().stream().map(Coupon::getId).collect(Collectors.toList()));
                // 发送到 kafka 中做异步处理 同步修改DB的过期优惠卷状态
                kafkaTemplate.send(Constant.TOPIC, JSON.toJSONString(couponKafkaMessage));
            }
            //如果没有过期的直接返回可用的
    ...
    public SettlementInfo settlement(SettlementInfo info) throws CouponException {
        ...
        if (processedInfo.getEmploy() && CollectionUtils.isNotEmpty(processedInfo.getCouponAndTemplateInfos()
            ...
            CouponKafkaMessage couponKafkaMessage = new CouponKafkaMessage(CouponStatus.USED.getCode(), settleCoupons.stream().map(Coupon::getId).collect(Collectors.toList()));
            // 更新 db
            kafkaTemplate.send(Constant.TOPIC, JSON.toJSONString(couponKafkaMessage));
        }
